...
func (s *Service) Update(ctx context.Context, command SaveCommand, actorID int64) error {
	if command.ID <= 0 || command.ParentID == command.ID {
		return apperror.InvalidArgument(apperror.CodeInvalidArgument, "部门层级无效", nil)
	}
	...
	if command.ParentID != 0 {
		...
		if descendant {
			return apperror.InvalidArgument(apperror.CodeInvalidArgument, "不能将部门移动到自己的下级", nil)
		}
	}
	item, err := s.prepare(ctx, command)
	if err != nil {
...
func (s *Service) Delete(ctx context.Context, id, actorID int64) error {
	if id <= 0 {
		return apperror.InvalidArgument(apperror.CodeInvalidArgument, "部门ID无效", nil)
	}
	...
	if children || accounts {
		return apperror.Conflict(apperror.CodeConflict, "部门存在下级部门或用户，不能删除")
	}
...
func (s *Service) prepare(ctx context.Context, command SaveCommand) (domain.Department, error) {
	...
	if err := item.Validate(); err != nil {
		return domain.Department{}, apperror.InvalidArgument(apperror.CodeInvalidArgument, "部门名称、编码或状态无效", err)
	}
	...
	if codeExists || nameExists {
		return domain.Department{}, apperror.Conflict(apperror.CodeConflict, "部门名称或编码已存在")
	}
	item.TreePath = "0"
	if item.ParentID != 0 {
