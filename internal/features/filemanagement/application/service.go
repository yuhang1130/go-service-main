...
func (s *Service) Upload(ctx context.Context, name string, size int64, content io.Reader) (domain.File, error) {
	...
	if err != nil {
		return domain.File{}, apperror.InvalidArgument(apperror.CodeInvalidArgument, "文件类型或大小无效", err)
	}
	prefix := make([]byte, 512)
	read, readErr := io.ReadFull(content, prefix)
	if readErr != nil && !errors.Is(readErr, io.EOF) && !errors.Is(readErr, io.ErrUnexpectedEOF) {
		return domain.File{}, apperror.InvalidArgument(apperror.CodeInvalidArgument, "文件内容无效", readErr)
	}
	if read == 0 {
		return domain.File{}, apperror.InvalidArgument(apperror.CodeInvalidArgument, "文件内容为空", nil)
	}
	...
	if err := domain.ValidateContentType(extension, detectedType); err != nil {
		return domain.File{}, apperror.InvalidArgument(apperror.CodeInvalidArgument, "文件内容与扩展名不匹配", err)
	}
	...
	if err != nil {
		if errors.Is(err, ErrTooLarge) {
			return domain.File{}, apperror.InvalidArgument(apperror.CodeInvalidArgument, "文件超过大小限制", nil)
		}
...
func (s *Service) UploadImage(ctx context.Context, name string, size int64, content io.Reader) (domain.File, error) {
	if err := domain.ValidateImage(name); err != nil {
		return domain.File{}, apperror.InvalidArgument(apperror.CodeInvalidArgument, "图片格式无效", err)
	}
...
func (s *Service) Open(ctx context.Context, key string) (io.ReadCloser, int64, error) {
	...
	if err != nil {
		return nil, 0, apperror.NotFound(apperror.CodeNotFound, "文件不存在")
	}
...
func (s *Service) Delete(ctx context.Context, raw string) error {
	...
	if err != nil {
		return apperror.InvalidArgument(apperror.CodeInvalidArgument, "文件路径无效", err)
	}
	if err := s.storage.Delete(ctx, key); err != nil {
		if errors.Is(err, ErrNotFound) {
			return apperror.NotFound(apperror.CodeNotFound, "文件不存在")
		}
